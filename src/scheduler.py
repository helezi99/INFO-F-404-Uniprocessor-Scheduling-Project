# src/scheduler.py
from .algorithms import EDF, RoundRobin
...
class Scheduler:
    ...
    def _calculate_simulation_length(self):
        ...
        For Round Robin: uses hyperperiod P
        For EDF: uses workload-based iteration method
        """
        if isinstance(self.algorithm, RoundRobin):
            # 计算所有任务周期的最小公倍数
            from math import lcm  # Python 3.9+
            return lcm(*(task.period for task in self.taskset))
        
        if isinstance(self.algorithm, EDF):
            # 对于EDF，使用原有的迭代方法
